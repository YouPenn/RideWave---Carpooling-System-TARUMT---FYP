...
// Redirect if no driver ID is found
if (!$driverID) {
    echo "<script>
...
// Fetch all payment records for trips created by this driver
$sql_payments = "SELECT p.paymentID, p.status, p.passengerConfirmDate, p.driverConfirmDate, b.bookingID, t.tripID, t.startLocation, t.endLocation, t.startDateTime 
                 FROM payment p
...
if ($stmt = $mysqli->prepare($sql_payments)) {
    ...
    while ($row = $result->fetch_assoc()) {
        if ($row['status'] === 'completed' || $row['status'] === 'absent' || $row['status'] === 'absent(refunded)' ) {
    ...
    <div class="content-wrapper">
        <div class="container-xxl flex-grow-1 container-p-y">   
            <h4 class="fw-bold py-3 mb-4">
                <span class="text-muted fw-light">Driver /</span> Payment List
            </h4> 
...
<td><?php echo strtoupper(htmlspecialchars($payment['status'])); ?></td>
                                            <td>
                                                <a href="payment_driver_confirm.php?bookingID=<?php echo urlencode($payment['bookingID']); ?>" class="btn btn-primary btn-sm">Confirm</a>
                                            </td>
                                        </tr>
                                    <?php endforeach; ?>
                                <?php endif; ?>
            ...
            <!-- Completed Payments -->
            <div class="card mb-4">
                <h5 class="card-header">Completed Payments</h5>
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-bordered">
                          <thead>
